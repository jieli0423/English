...
import { dashboardData, levelConfig, vocabularyData } from '../data/mockData'
...
export default function Dashboard() {
  ...
  const totalMastered = vocabularyData.filter(w => w.masterLevel >= 2).length
  ...
  const statsCards = [
    { icon: '🔥', label: '连续学习天数', value: stats.streak, unit: '天', bgColor: 'bg-amber-50', trend: 12 },
    { icon: '⭐', label: '当前等级', value: stats.level, bgColor: 'bg-indigo-50' },
    { icon: '✅', label: '今日完成率', value: `${Math.round((completedCount / tasks.length) * 100)}%`, bgColor: 'bg-emerald-50' },
    { icon: '📖', label: '单词掌握数', value: stats.wordsMastered, unit: '词', bgColor: 'bg-violet-50', trend: 23 },
    { icon: '🖊️', label: '阅读正确率', value: `${stats.readingAccuracy}%`, bgColor: 'bg-blue-50' },
  ]
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {/* Stats Grid: 5 cards — responsive: 2/3/5 columns */}
      ...
      <div className="card p-5 sm:p-6 mb-8 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white border-0 shadow-lg shadow-indigo-600/20">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-white text-indigo-700 rounded-lg font-semibold text-sm hover:bg-indigo-50 active:scale-[0.97] transition-all duration-150 shadow-lg shadow-indigo-900/20 flex items-center gap-2"
          >
            继续学习
        ...
        <div className="lg:col-span-1 card p-5 sm:p-6">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="min-w-0">
              <p className="font-semibold text-slate-900 truncate">{levelConfig.title}</p>
          ...
          <div className="flex items-center justify-between mt-2 text-xs text-slate-400">
            <span>{levelConfig.milestones.filter(m => m.level <= levelConfig.current).slice(-2)[0]?.title || '开始'}</span>
            <span>→</span>
        ...
        <div className="lg:col-span-2 card p-5 sm:p-6">
          ...
          <div className="space-y-0">
            {recentRecords.map((record, i) => (
              <div key={record.id} className="flex items-start gap-3 py-3 border-b border-slate-50 last:border-b-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-medium text-slate-800 truncate">{record.action}</span>
                    <span className="text-xs text-slate-400 shrink-0">{record.time}</span>
                  </div>
                  <p className="text-xs text-slate-500 mt-0.5 truncate">{record.detail}</p>
                </div>
                <span className="text-xs text-slate-400 shrink-0 mt-1 hidden sm:inline">{record.duration}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        <div className="lg:col-span-2 card p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-400">{completedCount}/{tasks.length} 已完成</span>
          </div>
          ...
          <div className="space-y-4">
            {recommendations.map((rec) => (
              <button
                ...
                className="w-full text-left p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-indigo-200 hover:shadow-md active:scale-[0.98] transition-all duration-150 group"
              >
